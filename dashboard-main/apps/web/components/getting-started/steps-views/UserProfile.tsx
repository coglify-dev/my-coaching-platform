...
const UserProfile = (props: IUserProfileProps) => {
  ...
  const mutation = trpc.viewer.updateProfile.useMutation({
    onSuccess: async (_data, context) => {
      if (context.avatar) {
        ...
        await utils.viewer.me.refetch();
        router.push("/auth/coach-verification"); //redirect to coach verification page till an admin verify the coach
      }
    },
    onError: () => {
  ...
  const DEFAULT_EVENT_TYPES = [
    ...
    // @WARNING: This is commented out because we don't want to show this to users yet
    // {
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      <Button
        type="submit"
        color="primary"
        className=" mt-8 flex w-full flex-row justify-center rounded-md p-2 text-center text-sm">
        {t("finish")}
